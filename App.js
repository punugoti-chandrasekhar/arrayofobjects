...
function App() {
  ...
  const downloadEmiStatement = () => {
    const emiData = emi
      .map(
        (entry) =>
          `${entry.month},${entry.principal},${entry.interest},${entry.balance}`
      )
      .join("\n");
              ...
              <div className="mt-4">
                <Row>
                  ...
                  <Col xs={6} className="text-left">
                    ...
                    <h6 style={{ color: "darkblue" }}>
                      ₹{" "}
                      {(+totalPayment - +principal).toLocaleString("en-IN", {
                        maximumFractionDigits: 2,
                      })}{" "}
                      /-
                    </h6>
      ...
      {emi.length > 0 && (
        <div className="mt-4" style={{ overflowY: "auto", maxHeight: "570px" }}>
          ...
          <Table
            ...
            hover
            size="sm"
            className="text-center table-sm-font-size"
          >
            <thead>
              <tr>
                <th>Month</th>
